...
const RequirementSection: React.FC<RequirementsProps> = ({ programUuid }) => {
  const { data: requirements, isLoading, isError } = useGetAllRequirementsQuery(programUuid
      ...
      {requirements.map((requirement: RequirementsType, idx: number) => (
        <div key={requirement.id || idx} className="grid gap-6">
          ...
          {requirement.subtitle && (
            <p className="text-sm sm:text-base md:text-lg">{requirement.subtitle}</p>
          )}

          {requirement.description && requirement.description.length > 0 && (
            <ul className="list-disc list-inside grid gap-4 sm:gap-6 text-foreground text-sm sm:text-base md:text-lg font-normal shadow-[0_4px_15px_rgba(0,0,0,0.15)] border-l-4 border-secondary-hover p-4 sm:p-6 md:p-8 rounded-lg">
