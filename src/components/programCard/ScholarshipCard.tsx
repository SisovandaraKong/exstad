...
const ScholarshipCard: React.FC<programType> = ({
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2  rounded-[24px] justify-between gap-4 p-[24px] bg-background  [box-shadow:0px_8px_24px_rgba(0,0,0,0.05)] '>
        <Link href={`/explore-course/${id}`}><Image unoptimized height={500} width={500} src={image} alt={title} className="h-fit rounded-[20px]" /></Link>     
        <div className='h-full flex flex-col gap-y-[10px]'>
            <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className='flex flex-col '>
                     <Link href={`/explore-course/${id}`}> <h1 className=" text-primary text-[30px] font-bold hover:text-primary-hover"> {title}</h1> </Link>
